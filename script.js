...
function showPeople() {
  ...
  people.forEach((person) => {
    ...
    /**TO DO: Write JavaScript code to add each character's information onto the webpage**/
      ...
      personImg.src = person.fields.cover_image[0].url;
      peopleContainer. appendChild(personImg);

      const personNickname = document.createElement("h2");
      personNickname.innerText = person.fields.nickname;
      peopleContainer.appendChild(personNickname);

      const personName = document.createElement("h3");
      personName.innerText = person.fields.name;
      peopleContainer.appendChild(personName);

      const personActor = document.createElement("h3");
      personActor.innerText = person.fields.played_by;
      peopleContainer.appendChild(personActor);
